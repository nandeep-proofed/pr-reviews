...
**Status at a glance:** #1 ✅ Fixed · #2 ⏸️ Deferred · #3 ✅ Fixed · #4 ⏸️ Deferred (pre-existing)
...
Better still, add a cross-field check to `submitReturnJobRequestSchema` so `body.id` must equal `query.jobId`, which closes the pre-existing ambiguity for every consumer in the route. **Applied:** the `jobUpdateData.id` change landed in `6b9267d97`, pinned by a test that passes a divergent query id and was verified to fail against the old code. The schema cross-check was not added — the pre-existing `fetchJobTasksByJobId` divergence therefore still stands.
...
**Status:** ⏸️ Deferred — accepted as-is for this PR; now stated explicitly in the PR description instead of left silent. Follow-up ticket if bulk return submits ever run at scale.
...
**Status:** ✅ Fixed in `6b9267d97` — comments restored on both blocks and verified present in the committed blob. Root cause of the original disappearance still unexplained; it did not recur.
...
**Status:** ⏸️ Deferred — not this PR's code and not this PR's regression. Needs its own ticket; it cost this session real time and will cost the next person the same.
...
- ✅ 9 tests added across the two changed routes — meets the project requirement that every PR includes tests for new code (10 after the Issue 1 fix)
...
- ✅ ~~No test covers the query-`jobId`-vs-body-`id` divergence in Issue 1~~ — added in `6b9267d97`
...
| Code quality | ✅ Issues 1 and 3 fixed in `6b9267d97`; Issue 2 knowingly deferred and documented |
...
1. ✅ **Strike the 409 warning from the PR description** — done; struck with the correction and the real residual question recorded in its place.
2. ✅ **Fix Issue 1** — done in `6b9267d97` (`jobId: jobUpdateData.id`), with a test.
3. ✅ **Restore the Issue 3 comments** — done in `6b9267d97`, verified in the committed blob.
4. ⏳ **QA on b2btest against job 24458 specifically**, watching for whether OMS accepts a past-dated `returnTime`. Cover the bulk toolbar's "Mark as Returned" too, since this PR changes it; if `getBulkActionsConfig` cannot pass an In Queue/On Hold job to that route, say so and the bulk half can be treated as a no-op. **Open — the only item still outstanding.**
5. ⏸️ **Optional follow-ups, not for this PR:** the bulk prefetch (Issue 2) and the misleading 409 message (Issue 4). Neither is ticketed yet.
